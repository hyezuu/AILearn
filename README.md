...
AILearn(에일리언)은 AI 기술을 활용한 혁신적인 온라인 영어 학습 플랫폼입니다. 
Alan AI를 통해 개인화된 학습 경험을 제공하며, 사용자의 영어 실력 향상을 효과적으로 지원합니다.

## 🚩 프로젝트 개요
- 프로젝트 이름: AILearn (에일리언)
- 프로젝트 기간: 2024.08.21 ~ 2024.09.12
- 배포 주소: [AILearn✨](http://your-deployment-url-here/)
...
5. **쓰기 연습**: AI 첨삭 기능으로 작문 실력을 향상시킵니다.
6. **게임화 요소**: 일일 챌린지와 퀘스트로 학습 동기를 부여합니다.
7. **이메일 인증**: 안전한 회원가입을 위한 이메일 인증 기능을 제공합니다.
8. **임시 비밀번호 발급**: 비밀번호 분실 시 이메일로 임시 비밀번호를 전송합니다.

## 👀 팀 소개

| 이름 | 역할 | 담당 업무 |
|------|------|-----------|
| 팀원1 | Frontend / Backend | - 기능1<br>- 기능2 |
| 팀원2 | Backend | - 기능3<br>- 기능4 |
| 팀원3 | Frontend | - 기능5<br>- 기능6 |
| 팀원1 | Frontend / Backend | - 기능1<br>- 기능2 |
| 팀원2 | Backend | - 기능3<br>- 기능4 |
| 팀원3 | Frontend | - 기능5<br>- 기능6 |
| 팀원1 | Frontend / Backend | - 기능1<br>- 기능2 |
...
### 1.1 회원가입
- **설명**: 사용자가 회원가입을 통해 계정을 생성할 수 있어야 한다.
- **엔드포인트**: `POST /api/signup`
- **요구사항**:
  - 사용자 이름, 이메일, 비밀번호 등의 필수 정보를 입력하여 회원가입을 완료해야 한다.
  - 이메일 중복 확인 후 가입을 진행해야 한다.

### 1.2 회원 탈퇴
- **설명**: 사용자가 자신의 계정을 삭제할 수 있어야 한다.
- **엔드포인트**: `DELETE /api/withdrawal`
- **요구사항**:
  - 로그인한 사용자는 본인의 계정을 삭제할 수 있어야 한다.

### 1.3 이메일 및 닉네임 중복 확인
- **설명**: 회원가입 시 이메일 및 닉네임 중복 여부를 확인할 수 있어야 한다.
- **엔드포인트**: 
  - `GET /api/email-duplication?email={email}`
  - `GET /api/nickname-duplication?nickname={nickname}`
- **요구사항**:
  - 회원가입 시 입력된 이메일 및 닉네임의 중복 여부를 확인하여 중복된 경우 오류 메시지를 표시해야 한다.

### 1.4 이메일 인증
- **설명**: 회원가입 시 이메일 인증 절차를 통해 계정을 활성화해야 한다.
- **엔드포인트**: 
  - `POST /api/request-verification?email={email}`
  - `POST /api/verify-email?email={email}&code={code}`
- **요구사항**:
  - 회원가입 시 입력된 이메일로 인증 코드를 전송하고, 사용자가 해당 코드를 입력하여 이메일을 인증해야 한다.

### 1.5 회원 정보 조회 및 수정
- **설명**: 사용자는 자신의 회원 정보를 조회하고 수정할 수 있어야 한다.
- **엔드포인트**: 
  - `GET /api/me`
  - `PUT /api/me`
- **요구사항**:
  - 사용자는 자신의 개인정보(닉네임, 비밀번호 등)를 조회하고 수정할 수 있어야 한다.
...
### 2.1 로그인 및 로그아웃
- **설명**: 사용자는 이메일과 비밀번호를 통해 로그인하고, 로그아웃할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/auth/login`
  - `GET /api/auth/logout`
- **요구사항**:
  - 로그인 시 유효한 이메일과 비밀번호로 인증해야 한다.
  - 로그아웃 후 세션 또는 토큰이 만료되어야 한다.

### 2.2 비밀번호 찾기
- **설명**: 사용자는 이메일을 통해 비밀번호 찾기 요청을 할 수 있어야 한다.
- **엔드포인트**: `GET /api/auth/password`
- **요구사항**:
  - 사용자가 비밀번호를 찾을 수 있도록 이메일을 통한 비밀번호 초기화 절차를 제공해야 한다.
...
### 3.1 게시글 작성 및 수정
- **설명**: 사용자는 게시글을 작성하고 수정할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/post`
  - `PUT /api/posts/{id}`
- **요구사항**:
  - 사용자는 게시글을 작성하고, 본인이 작성한 게시글을 수정할 수 있어야 한다.

### 3.2 게시글 조회 및 삭제
- **설명**: 사용자는 게시글 목록을 조회하고, 단일 게시글을 조회 및 삭제할 수 있어야 한다.
- **엔드포인트**:
  - `GET /api/posts`
  - `GET /api/posts/{id}`
  - `DELETE /api/posts/{id}`
- **요구사항**:
  - 사용자는 모든 게시글 목록을 조회할 수 있어야 하며, 특정 게시글의 상세 내용을 확인할 수 있어야 한다.
  - 게시글 작성자는 본인의 게시글을 삭제할 수 있어야 한다.

### 3.3 내 게시글 목록 조회
- **설명**: 사용자는 자신이 작성한 게시글 목록을 조회할 수 있어야 한다.
- **엔드포인트**: `GET /api/me/posts`
- **요구사항**:
  - 사용자는 본인이 작성한 게시글 목록을 조회할 수 있어야 한다.
...
### 4.1 게시글 좋아요 기능
- **설명**: 사용자는 게시글에 '좋아요'를 추가하거나 취소할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/posts/{id}/like`
  - `DELETE /api/posts/{id}/like`
- **요구사항**:
  - 게시글에 대해 '좋아요' 기능을 제공하며, 사용자는 게시글에 좋아요를 누르거나 취소할 수 있어야 한다.

### 4.2 좋아요한 게시글 목록 조회
- **설명**: 사용자는 자신이 좋아요한 게시글 목록을 조회할 수 있어야 한다.
- **엔드포인트**: `GET /api/me/likes`
- **요구사항**:
  - 사용자는 자신이 좋아요를 누른 게시글 목록을 확인할 수 있어야 한다.
...
### 5.1 댓글 작성 및 삭제
- **설명**: 사용자는 게시글에 댓글을 작성하고 삭제할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/posts/{id}/comments`
  - `DELETE /api/posts/{id}/comments/{id}`
- **요구사항**:
  - 사용자는 게시글에 댓글을 달 수 있으며, 본인이 작성한 댓글을 삭제할 수 있어야 한다.

### 5.2 댓글 목록 조회
- **설명**: 사용자는 특정 게시글에 달린 댓글 목록을 조회할 수 있어야 한다.
- **엔드포인트**: `GET /api/posts/{id}/comments`
- **요구사항**:
  - 사용자는 특정 게시글에 달린 모든 댓글을 조회할 수 있어야 한다.

### 5.3 내 댓글 목록 조회
- **설명**: 사용자는 자신이 작성한 댓글 목록을 조회할 수 있어야 한다.
- **엔드포인트**: `GET /api/me/comments`
- **요구사항**:
  - 사용자는 본인이 작성한 댓글 목록을 조회할 수 있어야 한다.
...
### 6.1 단어장 관리
- **설명**: 사용자는 자신의 단어장을 조회, 생성, 수정, 삭제할 수 있어야 한다.
- **엔드포인트**:
  - `GET /api/me/vocabulary-list`
  - `POST /api/vocabulary-list`
  - `POST /api/vocabulary-list/me/vocabularies`
  - `DELETE /api/vocabulary-list/me/vocabularies/{id}`
- **요구사항**:
  - 사용자는 자신의 단어장을 생성하고, 단어장을 관리할 수 있어야 한다.
  - 단어 추가 및 삭제 기능을 통해 단어장을 업데이트할 수 있어야 한다.
...
### 7.1 레벨테스트 조회 및 제출
- **설명**: 사용자는 레벨테스트를 보고, 정답을 제출할 수 있어야 한다.
- **엔드포인트**:
  - `GET /api/tests`
  - `POST /api/grade`
- **요구사항**:
  - 사용자는 레벨테스트의 문제를 조회하고, 정답을 제출할 수 있어야 한다.

### 7.2 승급 테스트
- **설명**: 사용자는 승급 테스트 문제를 보고, 정답을 제출할 수 있어야 한다.
- **엔드포인트**: `POST /api/upgrade`
- **요구사항**:
  - 승급 테스트를 통해 사용자가 상위 레벨로 승급할 수 있어야 한다.
...
### 8.1 문법 예문 관리
- **설명**: 사용자는 문법 예문을 생성, 조회, 채점할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/grammar-examples`
  - `GET /api/me/grammar-examples`
  - `POST /api/grammar-examples/{id}/grading`
  - `POST /api/grammar-examples/more`
- **요구사항**:
  - 사용자는 문법 예문을 작성하고, 예문을 조회하거나 추가할 수 있어야 한다.
  - 문법 예문에 대한 자동 채점 기능이 제공되어야 한다.
...
### 9.1 에세이 관리
- **설명**: 사용자는 에세이를 작성하고, 수정 및 조회할 수 있어야 한다.
- **엔드포인트**:
  - `POST /api/essays`
  - `PUT /api/essays/{id}`
  - `GET /api/me/essays`
...
<img width="565" alt="스크린샷 2024-09-09 오후 1 55 15" src="https://github.com/user-attachments/assets/1684f43c-4510-4ae8-b42a-ee36c430753f">
